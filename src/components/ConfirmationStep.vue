<template>
  <div>
    <h2 class="headline mb-4">Eine letzte Überprüfung Ihrer Daten zur Reservierung</h2>
    <p class="body-1 mb-4">Sollten Sie bla Info</p>
    <v-container fluid grid-list-xl>
      <v-layout row wrap>
        <v-flex v-for="(patient, index) in patients"
          :key="index" xs12 sm6 md4>
          <v-card>
            <v-card-title>
              <div class="headline">{{ patient.slots[0].time | moment('DD.MM. HH:mm') }}</div>
            </v-card-title>
            <v-divider />
            <v-list dense>
              <v-list-tile>
                <v-list-tile-content>Termintyp:</v-list-tile-content>
                <v-list-tile-content class="align-end">{{ patient.slots[0].calendar.title }}</v-list-tile-content>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-content>Name:</v-list-tile-content>
                <v-list-tile-content class="align-end">{{ patient.firstName }} {{ patient.lastName }}</v-list-tile-content>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-content>Email:</v-list-tile-content>
                <v-list-tile-content class="align-end">{{ patient.email }}</v-list-tile-content>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-content>Telefon:</v-list-tile-content>
                <v-list-tile-content class="align-end">{{ patient.phoneNumber }}</v-list-tile-content>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-content>SVNR:</v-list-tile-content>
                <v-list-tile-content class="align-end">{{ patient.insuranceNumber }}</v-list-tile-content>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-content>Geburtstag:</v-list-tile-content>
                <v-list-tile-content class="align-end">{{ patient.birthday | moment('DD.MM.YYYY') }}</v-list-tile-content>
              </v-list-tile>
            </v-list>
            <v-divider />
            <v-card-actions>
              ...
              <v-btn
                color="secondary"
                flat
                @click="editPatient(index, patient)">Bearbeiten</v-btn>
              <v-btn
                color="secondary"
                flat
                @click="deleteSlot(patient.slots[0])">Löschen</v-btn>
            </v-card-actions>
          </v-card>
        </v-flex>
        <v-flex xs12>
    ...
    <v-btn
      ...
      color="primary"
      :disabled="!valid"
      @click="completeReservation">Verbindlich reservieren</v-btn>
  </div>
</template>
